...
from django.template.base import TextNode, Node
...
VOID_ELEMENTS = ["<area ", "<base ", "<br", "<col ", "<command ", "<embed ", "<hr", "<img ", "<input ", "<keygen ",
...
class SchemaNode(template.Node):
    ...
    def assemble_for_node(self, node, context):
        ...
        my_model = apps.get_model(self.obj._meta.app_label, sub_obj)
        if not my_model:
            sfind = sub_obj.find(".")
            if sfind != -1:
                sub_obj = sub_obj[:sfind]
