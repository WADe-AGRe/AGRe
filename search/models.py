...
class CourseInstance(models.Model):
    courseMode = models.CharField(
        help_text='''The medium or means of delivery of the course instance or the mode of study, either as ''' +
    ...
    instructor = models.ForeignKey('Person', related_name='instructor',
                                   on_delete=models.CASCADE, blank=True)
    duration = models.CharField(help_text="	The duration of the item (movie, audio recording, event, etc.) .",
                                max_length=255)
    location = models.CharField(
        help_text="The location of for example where the event is happening, an organization is located, or where an action takes place.",
        max_length=255)
...
class Course(CreativeWork):
    ...
    educationalCredentialAwarded = models.TextField(
        help_text="A description of the qualification, award, certificate, diploma or other educational credential " +
...
class WebPage(CreativeWork):
    significantLink = models.CharField(
        help_text="One of the more significant URLs on the page. Typically, these are the non-navigation links that are clicked " +
